...
- 游戏详情/播放状态/编辑游玩时长等共享弹窗的 UI 元素 helper（`createLauncherDialog`/`createDialogRoot`/`createDialogTitle`/`createDialogButton`/`createDialogCancelButton`）位于 `com.apps.game.GameActionMenuFactory`。这些 helper 仅供 `GameActionMenuFactory` 的共享弹窗使用；新增同类弹窗必须直接用 `PadDialogFactory` 的对应 `show*()` API，不要扩展这些 helper 或手写 root/title/button。
