...
public class LauncherModuleCompatibilityActivity extends AppCompatActivity {
    ...
    private void refreshInstalledModules() {
        ...
        AppExecutors.runOnIo(() -> {
            boolean installed = ExternalRpgMakerPluginStrategy.isRpgMakerPluginInstalled(this);
            runOnUiThread(() -> {
                if (isFinishing() || isDestroyed()) return;
                rpgmModuleInstalled = installed;
                binding.moduleRpgmRow.setEnabled(true);
                binding.moduleRpgmRow.setAlpha(1f);
                int stateColor = installed ? LauncherTheme.primary(this) : LauncherTheme.danger(this);
                binding.moduleRpgmIcon.setImageTintList(ColorStateList.valueOf(stateColor));
            });
        });
    }
    ...
    private void openRpgmInstallPage() {
        try {
            Intent browserIntent = new Intent(Intent.ACTION_VIEW, Uri.parse(RPGM_INSTALL_URL));
