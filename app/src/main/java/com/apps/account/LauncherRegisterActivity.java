...
/**
 * 注册页薄宿主（重构计划 9.9 阶段 108）。
 *
 * 全部逻辑抽取至 {@link LauncherRegisterFragment}，本类仅承载竖屏独立启动路径
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        if (savedInstanceState == null) {
            getSupportFragmentManager().beginTransaction()
                    .replace(R.id.authHostContainer, LauncherRegisterFragment.newInstance())
                    .commit();
        }
    }

    /** 注册页成功/失败关闭入口：成功时回传 RESULT_OK（原 Activity setResult 语义）。 */
    void finishRegister(boolean resultOk) {
