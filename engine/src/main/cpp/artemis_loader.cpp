...
// 从 intent extra 读取 engineLibName（如 "artemis" / "artemis-compatible"）。
// 失败时返回空字符串。
...
// 解析 app 私有 filesDir 绝对路径，用于拼接待加载插件的 so 路径。
...
// 优雅结束当前 Activity：真实插件缺失时给出可读日志并 finish，避免静默黑屏。
// 直接使用 NDK 的 ANativeActivity_finish()，避免 JNI 反射开销与局部引用管理。
void finishActivity(ANativeActivity* activity) {
    ANativeActivity_finish(activity);
}

// engineLibName 硬白名单：只允许三套已知 revision 库名，杜绝路径穿越。
