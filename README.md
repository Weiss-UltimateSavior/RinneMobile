...
## 特性
- **多引擎支持**：内置kr ty ar ons引擎，外置模块支持RPGM Renpy Godot，支持外置跳转主流游戏环境
- **统一游戏库**：添加、编辑、删除并管理本地游戏、安卓应用和外部启动入口。
- **多界面适配**：提供 竖屏、横屏、大屏 三种界面布局，满足不同用户的 手机/平板 使用习惯。
- **统一游戏存档管理**：导入导出覆盖管理本地内置引擎的游戏存档。
- **游戏资料**：集成 VNDB 与 Bangumi 数据源，便于补充游戏信息。
- **多样主题**：提供多种风格 主题/首页/导航栏 样式以及背景动态粒子效果，满足不同用户视觉需求。
- **灵活启动**：支持空目录条目、包名启动、自定义快捷方式和多种启动方式。
- **本地智能体**：内置 AI 智能体，连接用户自有API，可查询游戏库、浏览与搜索游戏文件、辅助汉化文本替换等功能，功能将持续更新；两种权限模式，配置温度工具调用次数等。支持 MCP（Model Context Protocol）Streamable HTTP 服务器扩展工具能力。
- **数据同步**：支持游戏条目、游玩记录的导入、导出与同步；空目录条目可匹配并恢复。
- **智能翻译**：支持用户接入多模态模型进行游戏对话翻译。
- **使用保障**：内置免责声明，提供深色界面与横屏体验。
